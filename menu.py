...
# ──────────────────────────────────────────────────────────────────────────────
def draw_character(surf, cx, cy, size, body_color, hat_name, glasses_name="None", pet_name="None", is_p1=True):
    """
    Draw a plain coloured square with two eyes, a hat on top, and optional glasses.
    ...
    if pet_name != "None":
        ...
        # P1: right si pet; P2: left si pet
...
def draw_glasses_only(surf, cx, cy, size, glasses_name):
    """
    Draws a small preview icon for the glasses selection grid.
    Uses the PNG from assets/glasses/ when available, falls back to procedural.
    """
    if glasses_name == "None":
        font = pygame.font.SysFont("Arial", 18)
        lbl  = font.render("None", True, (160, 185, 230))
...
def draw_map_thumbnail(surf, rect, map_name):
    ...
    if img is not None:
        ...
        if map_name == "Castle Rock":
            ...
            pygame.draw.rect(surf, (12,  8, 22), (x + w // 2 - 14, y + h // 2, 28, h))
            ...
            for ix in range(x + 8, x + w, 16):
                pts = [(ix, y + h // 2), (ix + 7, y + h // 2 - 22), (ix + 14, y + h // 2)]
                pygame.draw.polygon(surf, (195, 228, 255), pts)
        elif map_name == "Rooftop Rush":
            for bx, bh in ((x + 5, h // 2), (x + w // 3, h * 2 // 5), (x + w * 2 // 3 - 5, h // 3)):
            ...
            for vx in range(x + 6, x + w, 22):
                pygame.draw.line(surf, (18, 75, 28), (vx, y), (vx + 6, y + h // 2), 2)
...
class PetButton:
    """2-column scrollable pet picker cell."""
    BTN_W = 110
    BTN_H = 110
...
class GoButton:
    ...
    def draw(self, surf, font):
        ...
        surf.blit(lbl, (r2.centerx - lbl.get_width() // 2, r2.centery - lbl.get_height() // 2))
...
class Particle:
    ...
    def draw(self, surf):
        ...
        pygame.draw.circle(s, (*CYAN, self.alpha), (int(self.r), int(self.r)), int(self.r))
...
def run_menu(screen=None):
    ...
    thumb_rect = pygame.Rect(
        map_panel.x + 28, map_panel.y + 50,
        map_panel.width - 56, 170
    )
    ...
    def draw_frame(dt):
        ...
        for (panel, pet_panel, label_txt, color_idx, hat_idx, glasses_idx, pet_idx,
            ...
            (p1_panel, p1_pet_panel, "Player 1", p1_color_idx, p1_hat_idx, p1_glasses_idx, p1_pet_idx,
            ...
            (p2_panel, p2_pet_panel, "Player 2", p2_color_idx, p2_hat_idx, p2_glasses_idx, p2_pet_idx,
            ...
            draw_rounded_rect(canvas, PANEL_BG, panel, radius=20, border=3, border_color=PANEL_RIM)

            badge_color = CYAN if is_p1 else GOLD
            badge_r     = pygame.Rect(panel.x + 30, panel.y - 16, panel.width - 60, 34)
            draw_rounded_rect(canvas, (16, 30, 65), badge_r, radius=12, border=2, border_color=badge_color)
            ...
            draw_character(canvas, panel.centerx, PREVIEW_CY, 115, body_rgb, hat_name, glasses_name, pet_name, is_p1)
            ...
            draw_rounded_rect(canvas, PANEL_BG, pet_panel, radius=16, border=2, border_color=PANEL_RIM)
        ...
        # ── map panel ────────────────────────────────────────────────────────
        draw_rounded_rect(canvas, PANEL_BG, map_panel, radius=20, border=3, border_color=PANEL_RIM)
        ...
        nb = pygame.Rect(thumb_rect.x + 10, thumb_rect.bottom - 32, thumb_rect.width - 20, 26)
    ...
    while running:
        ...
        for event in events:
            ...
            # name box typing
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if p1_name_box.collidepoint(event.pos):   active_name_box = "p1"
                elif p2_name_box.collidepoint(event.pos): active_name_box = "p2"
                else:                                      active_name_box = None
            ...
            # P1 hat buttons – only fire if button is visually inside the hat clip zone
            ...
            if total_glasses_rows > GLASSES_VISIBLE_ROWS:
                ...
                if p1_gl_dn_arr.handle_event(event):
                    p1_glasses_scroll = min(total_glasses_rows - GLASSES_VISIBLE_ROWS, p1_glasses_scroll + 1)
            ...
            if total_glasses_rows > GLASSES_VISIBLE_ROWS:
                ...
                if p2_gl_dn_arr.handle_event(event):
                    p2_glasses_scroll = min(total_glasses_rows - GLASSES_VISIBLE_ROWS, p2_glasses_scroll + 1)
            ...
            if event.type == pygame.MOUSEWHEEL:
                ...
                # Determine if mouse is in hat zone or glasses zone of a panel
                ...
                if p1_pet_panel.collidepoint(cmx, cmy):
                    if pt_zone_top <= cmy <= pt_zone_bottom and total_pet_rows > PET_VISIBLE_ROWS:
                        p1_pet_scroll = max(0, min(
                            total_pet_rows - PET_VISIBLE_ROWS,
                            p1_pet_scroll - event.y))
                if p2_pet_panel.collidepoint(cmx, cmy):
                    if pt_zone_top <= cmy <= pt_zone_bottom and total_pet_rows > PET_VISIBLE_ROWS:
                        p2_pet_scroll = max(0, min(
                            total_pet_rows - PET_VISIBLE_ROWS,
...
if __name__ == "__main__":
    result = run_menu()
    if result:
        p1_name, p1c, p1h, p1g, p2_name, p2c, p2h, p2g, m = result
        print(f"{p1_name} → color={p1c}  hat={p1h}  glasses={p1g}")
        print(f"{p2_name} → color={p2c}  hat={p2h}  glasses={p2g}")
        print(f"Map → {m}")
    pygame.quit()
    sys.exit()
